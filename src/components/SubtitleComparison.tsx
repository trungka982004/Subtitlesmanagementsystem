    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="space-y-2">
          <div className="sticky top-0 bg-blue-50 p-3 rounded-lg border border-blue-200">
            <h3 className="text-blue-900">{file1.name}</h3>
            <p className="text-blue-700">{file1.entries.length} entries</p>
          </div>
        </div>

        <div className="space-y-2">
          <div className="sticky top-0 bg-green-50 p-3 rounded-lg border border-green-200">
            <h3 className="text-green-900">{file2.name}</h3>
            <p className="text-green-700">{file2.entries.length} entries</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-h-[600px] overflow-y-auto space-y-3">
        {Array.from({ length: maxEntries }).map((_, index) => {
          ...
          return (
            <div
              key={index}
              className={`grid grid-cols-1 lg:grid-cols-2 gap-4 p-4 rounded-lg border ${
                hasTimingMismatch
                  ? 'bg-orange-50 border-orange-200'
                  : 'bg-white border-gray-200'
              }`}
            >
              <div className={`p-3 rounded ${entry1 ? 'bg-blue-50' : 'bg-gray-100'}`}>
                {entry1 ? (
                  <>
                    ...
                    <p className="text-gray-900 mb-2">{entry1.text}</p>
                    {entry1.translation && (
                      <p className="text-gray-700 italic border-t border-blue-200 pt-2 mt-2">
                        {entry1.translation}
                      </p>
                    )}
                  ...
                  <p className="text-gray-400 text-center py-4">No matching entry</p>
                )}
              </div>

              <div className={`p-3 rounded ${entry2 ? 'bg-green-50' : 'bg-gray-100'}`}>
                {entry2 ? (
                  <>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {hasTimingMismatch && (
                        <span className="px-2 py-1 bg-orange-200 text-orange-900 rounded">
                          Δ {timingDiff.toFixed(1)}s
                        </span>
                      )}
                    </div>
                    <p className="text-gray-900 mb-2">{entry2.text}</p>
                    {entry2.translation && (
                      <p className="text-gray-700 italic border-t border-green-200 pt-2 mt-2">
                        {entry2.translation}
                      </p>
                    )}
